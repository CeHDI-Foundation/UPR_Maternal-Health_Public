...
pacman::p_load_gh(
  "vdeminstitute/vdemdata", # for V-Dem (Varieties of Democracy) dataset
  "PPgp/wpp2024" 
...
# Civil Society Participation Index ordinal (D) (e_v2x_cspart)
# Core civil society index (D) (v2xcs_ccsi)
# Core civil society index ordinal (D) (e_v2xcs_ccsi_3C)

vdem_data <- vdem |> 
  filter(year>=2000) |> 
  select(
    country_name, country_text_id, year,
    v2x_cspart, e_v2x_cspart_3C,
    v2xcs_ccsi, e_v2xcs_ccsi_3C
  ) |> 
  mutate(
    e_v2x_cspart_3C_cat = factor(
      case_when(
        e_v2x_cspart_3C == 0 ~ "Low",
        e_v2x_cspart_3C == 0.5 ~ "Moderate",
        e_v2x_cspart_3C == 1 ~ "High"
      ),
      levels = c("High", "Moderate", "Low")),
    e_v2xcs_ccsi_3C_cat = factor(
      case_when(
        e_v2xcs_ccsi_3C == 0 ~ "Low",
        e_v2xcs_ccsi_3C == 0.5 ~ "Moderate",
        e_v2xcs_ccsi_3C == 1 ~ "High"
      ),
      levels = c("High", "Moderate", "Low"))
  ) |> 
...
GDP <- read_csv(here("data", "API_NY.GDP.PCAP.PP.KD_DS2_en_csv_v2_1423.csv"), skip = 4) |> 
  janitor::clean_names() |> 
  select(country_name, country_code, indicator_name, x1998, x2005, x2019, x2023) |> 
  rename(GDP_1998 = x1998, GDP_2005 = x2005, GDP_2019 = x2019, GDP_2023 = x2023)

GDP_long <- read_csv(here("data", "API_NY.GDP.PCAP.PP.KD_DS2_en_csv_v2_1423.csv"), skip = 4) |> 
  janitor::clean_names() |> 
  select(country_name, country_code, indicator_name, x1998:x2024) |> 
  pivot_longer(names_to = "year", values_to = "GDP_long", cols = x1998:x2024) |> 
  mutate(year = as.numeric(str_remove(year, "x")))
...
saveRDS(GDP_long, file = here("data", "API_data", "GDP_long.rds"))