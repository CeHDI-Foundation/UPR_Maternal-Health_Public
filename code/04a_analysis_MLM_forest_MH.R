...
# anova(MLM_MMR_MH, MLM_MMR_MH2) # compare models
summary(MLM_MMR_MH)
# paste0(round((exp(-0.002442)-1)*100,2),"%") # calculate the % change from raw estimates
performance::check_model(MLM_MMR_MH, verbose = TRUE)
plot(predictorEffects(MLM_MMR_MH))

# Visualise the model predictions ----
p_mmr <-marginaleffects::plot_predictions(
  MLM_MMR_MH, 
  condition = list(
    year = seq(0,18, by=1),
...
plot_mmr <- p_mmr |> 
  ...
  # scale_y_continuous(limits = c(4.5, 5.7))+
  labs(title = "Modelled trajectories of MMR",
       y="Estimated MMR (log-transformed)",
  ...
  # annotate("text", x = -1, y = Inf, label = "Pre-UPR", 
  #          hjust = 1, vjust = 1.5, fontface = "italic", 
  #          color = "grey40", size = 3.5) +
  # annotate("text", x = 1, y = Inf, label = "Post-review (from the first UPR cycle)", 
...
ggsave(plot = plot_mmr, "linear_plot_MMR.svg", width = 6.5, height =5)

# 1. Prepare Data & Run Regressions ---------------------------------------
outcome_var <- "MMR_long_log"

forest_data_MLM_MMR <- map_dfr(theme_vars, function(theme) {
...
forest_plot_data_MLM_MMR <- forest_data_MLM_MMR %>%
...
max_ci <- max(forest_plot_data_MLM_MMR$conf.high_pct)
min_ci <- min(forest_plot_data_MLM_MMR$conf.low_pct)
...
p_forest_enhanced_MMR_MLM <- forest_plot_data_MLM_MMR |> 
  ...
  theme(
    ...
    legend.position.inside = c(0.08, 0.98), legend.justification = c(0,1),
...
# 4. Save -----------------------------------------------------------------
print(p_forest_enhanced_MMR_MLM)
# Univariable
annot1 <- forest_plot_data_MLM_MMR |> 
  filter(model == "Univariable") |> 
...
# Multivariable
annot2 <-  forest_plot_data_MLM_MMR |> 
...
p_forest_enhanced_MMR_MLM+annot1+annot2+plot_layout(widths = c(1.7,1.3,1.3))

ggsave(here("output", "publication_figures", "forest_plot_recs_by_MMR_MLM.svg"), 
       # plot = p_forest_enhanced_MMR_MLM, 
