...
MLM_SBA_MH <- glmmTMB::glmmTMB(
  skilled_birth_long_p_squeezed ~ 
...
summary(MLM_SBA_MH)
# paste0(round((exp(0.092179)-1)*100,2),"%") # calculate the % change from raw estimates
performance::check_model(MLM_SBA_MH, verbose = TRUE)

# Visualise the model predictions ----
p_sba <- plot_predictions(
  MLM_SBA_MH, 
  condition = list(
...
plot_sba <- p_sba |> 
  ...
  labs(
    title = "Modelled trajectories of Skilled Birth Attendance",
    y = "Estimated SBA (%)",
  ...
  scale_fill_manual(values = c("#D55E00", "#0072B2"));plot_sba

ggsave(plot = plot_sba, "spline_plot_SBA.svg", width = 6.5, height =5)

# 1. Prepare Data & Run Regressions ---------------------------------------
outcome_var <- "skilled_birth_long_p_squeezed"

forest_data_MLM_SBA <- map_dfr(theme_vars, function(theme) {
...
forest_plot_data_MLM_SBA <- forest_data_MLM_SBA %>%
...
max_ci <- max(forest_plot_data_MLM_SBA$conf.high_pct)
min_ci <- min(forest_plot_data_MLM_SBA$conf.low_pct)
...
p_forest_enhanced_SBA_MLM <- forest_plot_data_MLM_SBA |> 
...
# 4. Save -----------------------------------------------------------------
print(p_forest_enhanced_SBA_MLM)
# Univariable
annot1 <- forest_plot_data_MLM_SBA |> 
  filter(model == "Univariable") |> 
...
# Multivariable
annot2 <-  forest_plot_data_MLM_SBA |> 
...
p_forest_enhanced_SBA_MLM+annot1+annot2+plot_layout(widths = c(1.7,1.3,1.3))

ggsave(here("output", "publication_figures", "forest_plot_recs_by_SBA_MLM.svg"), 
       # plot = p_forest_enhanced_SBA_MLM, 
